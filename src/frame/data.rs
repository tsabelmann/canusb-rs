...
impl TryFrom<&[u8]> for DataFrame {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        ...
        let identifier_format = match expected_format {
            IdentifierFormat::Standard => {
                if value[0] == b't' {
                    ...
                    return Err(DataFrameParseError::MessageStartError)
                }
            },
            IdentifierFormat::Extended => {
                if value[0] == b'T' {
                ...
                match std::str::from_utf8(&value[1..1+3]) {
                    Ok(string) => {
                        match u32::from_str_radix(string, 16) {
                            Ok(v) => v,
                            Err(_) => return Err(DataFrameParseError::IntegerParseError)
                        }
                    },
                    Err(_) => return Err(DataFrameParseError::Utf8Error)
                }
            },
            IdentifierFormat::Extended => {
        ...
        let has_timestamp = match value.len() {
            ...
            _ => {
                return Err(DataFrameParseError::InvalidSize)
            }
        };
                    ...
                    match std::str::from_utf8(&value[(5+2*dlc) as usize..(5+2*dlc+4) as usize]) {
                        Ok(string) => {
                            match u16::from_str_radix(string, 16) {
                                Ok(v) => v,
                                Err(_) => {
                                    return Err(DataFrameParseError::IntegerParseError);
                                }
                            }
                        },
                        Err(_) => return {
                            Err(DataFrameParseError::Utf8Error)
                        }
                    }
                },
                IdentifierFormat::Extended => {
                    match std::str::from_utf8(&value[(10+2*dlc) as usize..(10+2*dlc+4) as usize]) {
                        ...
                        Err(_) => {
                            return Err(DataFrameParseError::Utf8Error);
                        }
                    }
                },
            }
        ...
        // build frame
        let builder = DataFrameBuilder::new()
...
mod tests {
    ...
    fn parse_std_data_frame_001() {
        let frame: DataFrame = DataFrameBuilder::new()
            .can_id(0x123, IdentifierFormat::Standard)
            .dlc(0)
            .into();

        let string = "t1230\r";
        let parse_frame: DataFrame = string.as_bytes().try_into().unwrap();
